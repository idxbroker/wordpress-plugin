...
function enqueue_single_listing_scripts() {
	// Template Styles.
	wp_register_style( 'wplistings-google-fonts', '//fonts.googleapis.com/css?family=Open+Sans+Pro:300,700|EB+Garamond:400', [], '1.0' );
	wp_enqueue_style( 'font-awesome-5.8.2' );
	wp_enqueue_style( 'wplistings-google-fonts' );
	wp_enqueue_style( 'impb-classical-template', IMPRESS_IDX_URL . 'assets/css/listing-templates/single-listing-classical.min.css', [], '1.0' );
	// Template Scripts.
	...
	wp_enqueue_script( 'impb-classical-script', IMPRESS_IDX_URL . 'assets/js/listing-templates/single-listing-classical.min.js', [], '1.0', false );
			...
			<div id="listing-details">
				...
				foreach ( (array) $details_instance->property_details['col2'] as $label => $key ) {
					...
					if ( ! empty( $detail_value ) ) {
						printf( '<tr class="wp_listings%s"><th class="label">%s</th><td>%s</td></tr>', esc_html( $key ), esc_html( $label ), esc_html( $detail_value ) );
					}
				}
				echo '</tbody>';
				...
				if ( isset( $options['wp_listings_display_advanced_fields'] ) && $options['wp_listings_display_advanced_fields'] ) {
					...
					if ( ! empty( $adv_fields['col1'] ) || ! empty( $adv_fields['col2'] ) ) {
						...
						foreach ( $adv_fields['col1'] as $key => $value ) {
							if ( ! empty( $value ) ) {
								printf( '<tr class="wp_listings%s"><th class="label">%s</th><td>%s</td></tr>', esc_html( $key ), esc_html( get_adv_field_display_name( $key ) . ':' ), esc_html( $value ) );
							}
						}
						echo '</tbody>';
