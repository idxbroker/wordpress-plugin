<?php
	...
	class Help {
		...
		/**
		 * Begin creating setting_help function
		 *
		 * @since 2.5.10
		 */
		...
		/**
		 * Begin creating add_wrappers_help function
		 *
		 * @since 2.5.10
		 */
		...
		/**
		 * Begin creating tabs array
		 *
		 * @since 2.5.10
		 * @var array $tabs contains an array of menu items for the help dropdown menu.
		 */
		public $tabs = array(
		...
		/**
		 * Begin creating add_pages_help_tabs
		 *
		 * @since 2.5.10
		 */
		public function add_pages_help_tabs() {
			...
			$screen->add_help_tab(array(
				'id'         => $id,
				'title'      => __( $data[ 'title' ], 'idxbroker' ), // The $text arg must be a single string literal, not "$data['title']".
		...
		/**
		 * Begin creating add_settings_help_tabs
		 *
		 * @since 2.5.10
		 */
		...
		/**
		 * Begin creating prepare function.
		 *
		 * @since 2.5.10
		 * @param mixed $screen is the menu type (admin or admin bar).
		 * @param mixed $tab is the menu tab item.
		 */
		public function prepare( $screen, $tab ) {
			printf( esc_html(
				'<p>%s</p>',
				__(
					$tab['callback'][0]->tabs[ $tab[ 'id' ] ][ 'content' ], // The $text arg must be a single string literal, not "$tab['callback'][0]->tabs[$tab['id']]['content']".
					'idxbroker'
				) )
			);
		}
		/**
		 * Begin creating impress_settings_page function
		 *
		 * @since 2.5.10
		 * @return Boolan value
		 */
		public function impress_settings_page() {
			// rocessing form data without nonce verification.
			if ( ( ! empty( $_GET['page'] ) &&
		...
		/**
		 * Begin creating disable_glow function
