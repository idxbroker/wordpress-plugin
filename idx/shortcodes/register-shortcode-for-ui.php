...
/**
 * Begin Regist_Shortcode_For_Ui class.
 */
class Register_Shortcode_For_Ui {
	...
	/**
	 * Beging default_shortcodes function.
	 *
	 * @since 2.5.10
	 * @return array $shortcode_type array of shortcodes.
	 */
	...
	/**
	 * Begin get_shortcode_options function.
	 *
	 * @since 2.5.10
	 */
	public function get_shortcode_options() {
		...
		if ( empty( $system_links_check ) || ! empty( $system_links_check->errors ) ) {
			if ( empty( $system_links_check ) ) {
				...
				// All output should be run through an escaping function (see the Security sections in the WordPress Developer Handbooks), found '$system_links_check'.
		...
		// Words in hook names should be separated using underscores. Expected:  'idx_get_shortcode_options' , but found:  'idx-get-shortcode-options'.
		do_action( 'idx-get-shortcode-options' );
		wp_die();
	}

	/**
	 * Begin shortcode_preview function.
	 *
	 * @since 2.5.10
	 */
	...
	/**
	 * Begin idx_wrapper_tags function.
	 *
	 * @since 2.5.10
	 * @param text $shortcode output the markup for the idx wrapper tags.
	 * @return html $output contains the html markup for the wrapper tags.
	 */
	...
	/**
	 * Begin get_shourtcodes_for_ui function.
	 *
	 * @since 2.5.10
	 * @return array merged array of available shortcodes.
	 */
	...
	/**
	 * Begin show_link_short_codes function.
	 *
	 * @since 2.5.10
	 * @param int $link_type contains the assigned type value for the link type. [ 0 = system links, 1 = saved links ].
	 * @return mixed array of links or boolean false.
	 */
	...
	/**
	 * Begin get_system_link_html function.
	 *
	 * @since 2.5.10
	 * @param text $idx_link contains the href of the IDX link.
	 * @return array $available_shortcodes
	 */
	...
	/**
	 * Begin get_saved_link_html function.
	 *
	 * @since 2.5.10
	 * @param text $idx_link contains the href of the IDX link.
	 * @return array $available_shortcodes
	...
	public function get_saved_link_html( $idx_link ) {
		...
		$available_shortcodes .= '<option id=\"' . $link_short_code . '\" value=\"' . $idx_link->uid . '\">';
		$available_shortcodes .= $idx_link->linkTitle . '</option>'; // Object property "$linkTitle" is not in valid snake_case format, try "$link_title".
		return $available_shortcodes;
	}

	/**
	 * Begin get_widget_html function.
	 *
	 * @since 2.5.10
	 * @return html $available_shortcodes markup for shortcode list.
	 */
	...
	/**
	 * Begin get_omnibar function.
	 *
	 * @since 2.5.10
	 * @param text $shortcode contains the name of the shortcode.
	 * @return html $output markup for omnibar.
	 */
	...
	/**
	 * Begin get_lead_login function.
	 *
	 * @since 2.5.10
	 * @param text $shortcode contains the name of the shortcode.
	 * @return html $output markup for the lead login widget.
	 */
	...
	/**
	 * Begin get_lead_signup function.
	 *
	 * @since 2.5.10
	 * @param text $shortcode contains the name of the shortcode.
	 * @return html $output markup for the lead signup widget.
	 */
	...
	/**
	 * Begin get_property_showcase function.
	 *
	 * @since 2.5.10
	 * @param text $shortcode constains the name of the shortcode.
	 * @return html $output contains the markup for the property showcase widget.
	 */
	...
	/**
	 * Begin get_property_carousel function.
	 *
	 * @since 2.5.10
	 * @param text $shortcode contains the name of the shortcode.
	 * @return html $output contains the generated markup of the widget.
	 */
	public function get_property_carousel( $shortcode ) {
		...
		$output .= '</script>';
		$output .= '<script src=\"https://code.jquery.com/jquery-2.2.2.min.js\"></script>'; // Scripts must be registered/enqueued via wp_enqueue_script.
		$output .= '<script src=\"' . plugins_url( '../assets/js/owl2.carousel.min.js', dirname( __FILE__ ) ) . '\"></script>'; // Scripts must be registered/enqueued via wp_enqueue_script.

		return $output;
	}

	/**
	 * Beging get_agents_select_list function.
	 *
	 * @since 2.5.10
	 * @param int $agent_id is the ID number of the agent from the array.
	 * @return html || null contain the markup of the agent list or null if no agents are found.
	 */
